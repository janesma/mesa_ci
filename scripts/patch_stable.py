...
mesa = pygit2.Repository(repo)

master = mesa.lookup_reference('refs/heads/master').target
branch = mesa.lookup_reference('refs/heads/18.0').target
branch_point = mesa.merge_base(master, branch)
    ...
    for aline in commit.message.splitlines():
        ...
        if ("fixes:" in aline):
            try:
                ...
                broken_branch_point = mesa.merge_base(branch, broken)
...
stable_commits = {}
for commit in mesa.walk(branch):
    if commit.id == branch_point:
        break
    for aline in commit.message.splitlines():
        aline = aline.lower()
